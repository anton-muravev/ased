# -*- coding: utf-8 -*-
## INVERSION2 EXPERIMENT ON CIFAR10
...
parser.add_argument('--cifarpath', required=True, help="path to CIFAR10 dataset")
...
if __name__ == '__main__':
    ...
    for it in np.arange(resume+1,big_iterations):
        ...
        norms = np.linalg.norm(prototype,axis=1)
        if norms.mean() > invert_threshold:
            ...
            if inv_counter == protolimit:
                ...
                sys.exit(0)
            inv_counter+=1
            modcount = np.int(np.floor(np.sqrt(prototype.shape[0])))
            modidx = np.argsort(norms)[-modcount:]
            inverted = ased.invert_prototype(prototype[modidx], len(opLibrary),
                                             prob_bound)
            prototype[modidx] = inverted            
